    ...
    <section className="py-20 md:py-28 relative" id="test-process">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <ScrollReveal>
          <SectionHeader
            badge="How It Works"
            title="Scholarship Test Process"
                  ...
                  <div className={`relative flex items-start gap-6 md:gap-0 ${isEven ? "md:flex-row" : "md:flex-row-reverse"}`}>
                    <div className={`flex-1 ${isEven ? "md:pr-12 md:text-right" : "md:pl-12"}`}>
                      <div className="bg-card rounded-2xl border border-border/50 p-5 md:p-6 shadow-premium hover:shadow-card-hover transition-shadow">
                        <span className="text-xs font-semibold text-gold uppercase tracking-wider">Step {step.step}</span>
                        <h3 className="font-heading font-semibold text-foreground text-lg mb-2 mt-2">{step.title}</h3>
                        <p className="text-sm text-muted-foreground leading-relaxed">{step.description}</p>
                      </div>
                    </div>
                    <div className="absolute left-5 md:left-1/2 md:-translate-x-1/2 flex h-7 w-7 items-center justify-center rounded-full bg-gold shadow-gold z-10">
